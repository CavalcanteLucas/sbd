...
Existem valores de `id_período` (ex.: `A`) que se relacionam com mais e um valor de `dia` (ex.: `M`, `W`, `F`). Ou seja, como enunciado, períodos podem ocorrer em mais de um dia por semana. Por isso, `dia` é <ins>necessário</ins> para identificar unicamente um determinado período.
...
então tanto `dia` quanto `hora_inicial` são também <ins>necessários</ins> para garantir a unicidade das tuplas nessa tabela.
...
Sendo assim, além de `id_período`, `dia` e `hora_inicial` são <ins>suficientes</ins> para identificar uma tupla dessa relação sem duplicações.
